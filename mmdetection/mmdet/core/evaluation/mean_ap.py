...
def eval_map(det_results,
    ...
    # print("eval_map() in sar_ship called")
    ...
    for i in range(num_classes):
        ...
        # calculate Average Domain Accuracy
        ave_acc_list = []
        for i in range(len(annotations)):
            tp_nums = sum(tp[i][0]) # num of 1 == tp nums
            fp_nums = len(tp[i][0]) - tp_nums # num of 0 == fp nums
            fn_nums = len(annotations[i]["bboxes"]) - tp_nums # nums of gt - tp_nums == fn
            inf = np.finfo(np.float32).eps # avoid divide 0
            dzw = (tp_nums + inf) / (tp_nums + fn_nums + fp_nums + inf)
            ave_acc_list.append(dzw)
        ave_acc = sum(ave_acc_list) / len(ave_acc_list)
        ...
        # ad_acc = recalls*precisions/(recalls+precisions-recalls*precisions+eps)
    ...
    print_map_summary(
        mean_ap, eval_results, ave_acc, dataset, area_ranges, logger=logger)

    return mean_ap, eval_results, ave_acc
...
def print_map_summary(mean_ap,
    ...
    header = ['class', 'gts', 'dets','recall', 'ap',"f1","precision","aver_dacc","ave_acc"]
    for i in range(num_scales):
        if scale_ranges is not None:
            print_log(f'Scale range {scale_ranges[i]}', logger=logger)
        table_data = [header]
        for j in range(num_classes):
            ...
            aver_dacc = recalls[i, j]*presicions[i, j]/(recalls[i, j]+presicions[i, j]-recalls[i, j]*presicions[i, j]+1e-6)

            row_data = [
                label_names[j], num_gts[i, j], results[j]['num_dets'],f'{recalls[i, j]:.3f}',
                f'{aps[i, j]:.3f}',f'{f1:.3f}',f'{presicions[i, j]:.3f}',f'{aver_dacc:.3f}',f'{ave_acc:.4f}'
            ]
            table_data.append(row_data)
        table_data.append(['mAP', '', '', '', f'{mean_ap[i]:.3f}'])
        table = AsciiTable(table_data)
